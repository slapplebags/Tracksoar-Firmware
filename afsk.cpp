...
static RadioHx1 radio;    // The radio (class defined in config.h)
...
{ phase_delta = PHASE_DELTA_1200;
  ...
  go = true;
  afsk_flush();       // Prime the fifo
  afsk_timer_setup(); // Start timer (CPU-specific)
  radio.ptt_on();     // Key the radio
  afsk_timer_start(); // Start transmission
}

bool afsk_flush()
{ while (! afsk_is_fifo_full_safe())
  { if (packet_pos == afsk_packet_size) // If done sending packet
    ...
    { if ((packet_pos & 7) == 0)              // Load up next byte
      ...
      if ((current_byte & 1) == 0)
      { phase_delta ^= (PHASE_DELTA_1200 ^ PHASE_DELTA_2200); // Toggle tone (1200 <> 2200)
      }
    }
...
AFSK_ISR
{ if (afsk_is_fifo_empty()) {  // This is called at PLAYBACK_RATE Hz to load the next sample.
    if (go) {
      sample_overruns++;
    }
  }
