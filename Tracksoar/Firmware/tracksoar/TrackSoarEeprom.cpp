/*-----------------------------------------------------------------------------*
 * TrackSoarEeprom.cpp - Arduino library to support external I2C EEPROMs.            *
 *                                                                             *
 ...
 *                                                                             *
 * To use the TrackSoarEeprom library, the Arduino Wire library must also            *
 * be included.                                                                *
 *                                                                             *
...
//   eeprom_size_t enumeration in the TrackSoarEeprom.h file. (Most
...
TrackSoarEeprom::TrackSoarEeprom(eeprom_size_t deviceCapacity, byte nDevice, unsigned int pageSize, uint8_t eepromAddr)
{
...
//bus (other than EEPROM), call TrackSoarEeprom::begin() after any initialization
//calls for the other devices to ensure the intended I2C clock speed is set.
byte TrackSoarEeprom::begin(twiClockFreq_t twiFreq)
{
    Wire.begin();
...
byte TrackSoarEeprom::write(unsigned long addr, byte *values, unsigned int nBytes)
{
...
//a status of EEPROM_ADDR_ERR is returned. For I2C errors, the status
//from the Arduino Wire library is passed back through to the caller.
byte TrackSoarEeprom::read(unsigned long addr, byte *values, unsigned int nBytes)
{
    byte ctrlByte;
    byte rxStatus;
...
//a status of EEPROM_ADDR_ERR is returned. For I2C errors, the status
//from the Arduino Wire library is passed back through to the caller.
byte TrackSoarEeprom::write(unsigned long addr, byte value)
{
...
int TrackSoarEeprom::read(unsigned long addr)
{
    uint8_t data;
    int ret;
