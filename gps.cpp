...
static void parse_hdop(const char *token);
...
static const t_nmea_parser gga_parsers[] = {
  ...
  NULL,             // Longitude
  NULL,             // E/W
  NULL,             // Fix quality 0 = no fix, 1 = fix, 2 = DGPS fix, 3 = PPS fix
  parse_num_sats,   // Number of Satellites in View
  parse_hdop,       // Horizontal Dilution Of Position
  parse_altitude,   // Altitude in Meters
...
float gps_course = 0;       // degrees
float gps_speed = 0;        // knots
float gps_altitude = 0;     // Meters ASL
byte  gps_num_sats = 0;     // Satellites in View
float  gps_hdop = 0;
bool  gps_low_power_mode = false;

// Ublox definitions

// Cyclic (Power Save Mode)
...
{ // Thanks to Darkside for this code:
  ...
  int temp = atoi(token);       // convert our token to an integer
  if (temp < 0)   temp = 0;     // range check it's between 0-32 satellites
  if (temp > 32)  temp = 32;
  new_num_sats = temp;          // store value away
}

void parse_hdop(const char *token)
{ new_hdop = atof(token);
}
...
{ uint8_t b;
  ...
  //#ifdef DEBUG_GPS
  //  debugSerial.print(F(" * Reading ACK response: ");
  //#endif
...
void setGPS_PowerSaveMode()       // Power Save Mode
{ sendUBX(setPSM, sizeof(setPSM) / sizeof(uint8_t));
}

void setGps_MaxPerformanceMode()  //Set GPS for Max Performance Mode
...
{ int gps_success = 0;
...
#ifdef GPS_USING_UBLOX    // Setup for the uBLOX MAX-6/7/8

#ifdef GPS_POWER_PIN      // Check if we need to power up the GPS - N/A to Tracksoar
  pinMode(GPS_POWER_PIN,   OUTPUT);
  pin_write(GPS_POWER_PIN, LOW);
#endif

#ifdef GPS_POWER_SLEEP_TIME
  // Add a bit of a delay to allow GPS a chance to wake up (even if not powered via a digital pin)
  delay(GPS_POWER_SLEEP_TIME);
#endif

#ifdef GPS_LED_PIN        // N/A to Tracksoar
  ...
  // Finally turn off any NMEA sentences we don't need
  // (in this case it's everything except GGA and RMC)
  ...
  switch (c)
  {
    ...
    case '\n':
      ...
      if (num_tokens && our_checksum == their_checksum)
      {
        ...
        Serial.print(millis());
#endif
        // Return a valid position only when we've got two rmc and gga messages with the same timestamp.
        ...
        //
        // 2. We just processed a known (GGA/RMC) sentence. Suppose the contrary:
        //    after starting up this module, gga_time & rmc_time are equal (both are
        //    initialized to "") so (1) holds and we wrongly report a valid position.
        //
        // 3. The GPS has a valid fix. For some reason, the Venus 634FLPX reports
        //    24 deg N, 121 deg E (the middle of Taiwan) until a valid fix is acquired:
        //
            ...
            active)                               // Valid fix?
        {
          ...
          { // Check if we WERE in low-power mode and either lost the fix
            // or were partially obscured
            if (gps_low_power_mode)
            { // Yeah. Switch back to max performance until we get more satellites.
