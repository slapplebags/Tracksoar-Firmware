...
void ax25_send_byte(uint8_t a_byte)    // Wrap around send_byte
{ send_byte(a_byte);
#ifdef DEBUG_AX25
...
{ // Make Path = none if DP_PATH = 0 or > 99 and Altitude is > than entered altitude.
  if (DP_PATH == 0 || (DP_PATH > 99 && gps_altitude > DP_PATH))
  ...
  send_byte(0x03);  // Control field: 3 = APRS-UI frame
  send_byte(0xf0);  // Protocol ID: 0xf0 = no layer 3 data

#ifdef DEBUG_AX25
  Serial.println();
  Serial.println(F("DEBUG_AX25"));
  Serial.print('[');
  Serial.print(millis());
  Serial.print(F("] "));
  Serial.print(addresses[1].callsign);    // Source callsign
  if (addresses[1].ssid)
  { Serial.print('-');
    Serial.print((unsigned int)addresses[1].ssid);
  }
  Serial.print('>');
  Serial.print(addresses[0].callsign);    // Destination callsign
  if (addresses[0].ssid)
  { Serial.print('-');
...
{ uint16_t final_crc = crc;         // Save crc so it can be treated atomically
  send_byte(~(final_crc & 0xff));   // Send the CRC
  final_crc >>= 8;
  send_byte(~(final_crc & 0xff));
  ax25_send_flag();     // Signal the end of frame
#ifdef DEBUG_AX25
  Serial.println();
#endif
}

void ax25_flush_frame()      // Key the transmitter and send the frame
